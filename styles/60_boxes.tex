...
%
% Der Balken selbst setzt KEINEN Abstand. Genau der erste echte Follower
% verbraucht den Merker und setzt einmal \ZSFbarAfterGap. Damit gibt es keine
% Vorauszahlung ueber \parskip und keine spaetere Gegenkorrektur mehr.
%
% Boxen und weitere Balken rufen \ZSFTitleBarGapConsume direkt. Fuer freien
% Text laeuft derselbe Verbraucher im LaTeX-Hook `para/before`, also noch im
% vertikalen Modus. Zwischen Balken und erstem Follower steht \parskip deshalb
% voruebergehend auf null; der Verbraucher setzt stattdessen genau den einen
% positiven Balkenabstand und stellt \parskip noch vor dem Inhalt wieder her.
% Ein Hook koexistiert mit den Paket-Hooks von tcolorbox und ueberschreibt
% insbesondere nicht mehr das globale \everypar-Register.
%
% DER MERKER GEHOERT DER AEUSSEREN VERTIKALEN LISTE, und zwar dem Zeitpunkt,
% an dem der Follower dort GESETZT wird. Genau daran hing ein Fehler, der jede
% erste Box nach einem Balken traf:
%
% Eine Box verbrauchte den Merker ueber den `before`-Schluessel von tcolorbox.
% Den fuehrt tcolorbox aber erst bei \end aus — den Titel der Box setzt es
% dagegen schon bei \begin. Bis dahin stand der Merker also noch, und die erste
% Zeile, die eine Box ueberhaupt setzt, ist ihr Titel: Der Absatzstart des
% Titels loeste den `para/before`-Hook aus, und der schrieb \ZSFbarAfterGap,
% \nointerlineskip und die globale \parskip-Wiederherstellung INS TITELFELD.
% Dessen Hoehe hatte tcolorbox schon berechnet, und die Folge war ein Titel,
% der oben aus seiner Box herauslief — der Rahmen schnitt durch die Oberlaengen
% (nachgemessen: 2 pt Fehlbetrag im Titelstreifen).
%
% Deshalb verbraucht eine Box den Merker beim EINTRITT (\ZSF@claimBarGap in der
% Fabrik), bevor tcolorbox irgendetwas von ihr setzt. Damit kann kein Material
% im Inneren einer Box den Merker mehr sehen — auch nicht die Titelzeile der
% leisen Fassung, der erste Absatz des Inhalts oder eine verschachtelte Box,
% die den Abstand sonst in ihre Elternbox geschrieben haette.
%
% Ein Merker »wir sind in einer Box«, der den Hook stummschaltet, waere die
% halbe Loesung: Er kaeme fuer den Titel zu spaet (tcolorbox setzt ihn, bevor
% irgendein Box-Hook laeuft) und liesse die Verschachtelung offen.
\newif\ifzsfTitleBarPending
\newskip\ZSFTitleBarSavedParskip
\newcommand{\ZSFTitleBarLockOn}{%
  % Defensive Idempotenz: Ein direkter zweiter Balken muss den ersten zuerst
  % verbrauchen. Falls ein neuer Balkentyp diesen Vertrag einmal vergisst,
  % darf er wenigstens nicht den gespeicherten Dokument-\parskip mit dem
  % voruebergehenden Nullwert ueberschreiben.
  \ifzsfTitleBarPending\else
    \global\ZSFTitleBarSavedParskip=\parskip
    \global\parskip=0pt
...
\newcommand{\ZSFTitleBarLockOff}{%
  \ifzsfTitleBarPending
    \global\parskip=\ZSFTitleBarSavedParskip
    \global\zsfTitleBarPendingfalse
  \fi
}
\newcommand{\ZSFTitleBarGapConsume}{%
  \ifzsfTitleBarPending
    % Erneut direkt vor der Glue sperren: Ein Index-Write zwischen Balken und
    % Follower darf die Bindung nicht auftrennen.
    \penalty10000
    \vskip\ZSFbarAfterGap
    \nointerlineskip
    \ZSFTitleBarLockOff
  \fi
}
\AddToHook{para/before}[zsf/titlebar-lock]{%
  \ifzsfTitleBarPending
    % Der Absatzstart hat den voruebergehend auf null gesetzten \parskip
    % bereits verarbeitet. Ab hier darf der Dokumentwert wieder gelten.
    \ZSFTitleBarGapConsume
  \fi
}

% Der Eintritt eines BLOCKS in die aeussere Liste: Steht ein Balkenabstand aus,
% gehoert er diesem Block und wird hier gesetzt — vor allem, was der Block
% selbst an Material erzeugt (Begruendung im Kopfkommentar oben).
%
% Der Merker bleibt, dass dieser Block den Abstand geholt hat. Ihn erneut aus
% \ifzsfTitleBarPending zu lesen geht nicht mehr: Der ist danach verbraucht,
% und ohne das Gedaechtnis legte \ZSF@blockBefore seinen eigenen Vorabstand
% obendrauf — der Balkenabstand ERSETZT ihn (30_layout_spacing →
% \ZSFBoxBeforeSkip). Gruppenlokal, damit die Antwort pro Block gilt und eine
% verschachtelte Box sie nicht an ihre Elternbox weitergibt.
\makeatletter
\newif\ifZSF@barGapClaimed
\newcommand{\ZSF@claimBarGap}{%
  \ZSF@barGapClaimedfalse
  \ifzsfTitleBarPending
    \par
    \ZSFTitleBarGapConsume
    \ZSF@barGapClaimedtrue
  \fi
}
\makeatother
...
% als letztes Element einer Spalte stehenbleibt — ohne sie entstehen verwaiste
% Überschriften, die man von Hand nicht sinnvoll nachräumt.
...
% Einheitlicher Top-Spacing-Kontrakt für Kapitel- und Abschnittsbalken.
%   #1 = normaler Abstand davor
%   #2 = Reserve-Mechanismus dieser Balkenebene
%
% Folgt normaler Inhalt, muss \addvspace VOR der Reserve stehen, damit es per
% Max-Semantik mit dem after-skip der Vorbox kollabiert. Folgt dagegen direkt
% ein zweiter Balken, verbraucht er den ausstehenden gemeinsamen
% Balkenabstand; sein normaler Vorabstand entfaellt vollstaendig.
...
\newcommand{\ZSFTitleBarTopSpacing}[2]{%
  \par
  \ifzsfTitleBarPending
    \ZSFTitleBarGapConsume
  \else
...
%
% Der Abstand selbst steht absichtlich nicht hier. Der Balken markiert nur den
% ausstehenden Follower; dieser setzt \ZSFbarAfterGap ueber genau einen
% Verbraucher. So bleiben unsichtbare Index-Writes erlaubt, ohne dass
% Abstandszustand oder Bindung verloren gehen.
\newcommand{\ZSFTitleBarAfter}{%
  \par\penalty10000\ZSFTitleBarLockOn
}
...
\tcbset{
  ...
  zsfbar/.style={
    ...
    right=\ZSFboxPadX,
    before skip=0pt,
    % HIER KEIN `after skip`. Der Balken setzt nur den Pending-Merker; genau
    % der erste sichtbare Follower besitzt und setzt \ZSFbarAfterGap. Ein
    % tcolorbox-`after skip` schriebe daneben einen zweiten Abstand in dasselbe
    % `after`-Makro und machte den Wert wieder vom folgenden \parskip abhaengig.
    after={\par},
  },
}
...
\newcommand{\ChapterBar}[1]{
  \ZSFTitleBarTopSpacing{\ZSFspaceL}{\BreakIfNotEnoughSpaceChapter}%
  \begin{chapterbar}
...
\newcommand{\ZSF@subsectionBarBody}[3]{%
  \ZSFTitleBarTopSpacing{\ZSFsubsectionBarBeforeSkip}{\BreakIfNotEnoughSpace}%
  \ZSFBarPenalty{-500}%
...
\newcommand{\ZSF@subsubsectionBarBody}[3]{%
  \ZSFTitleBarTopSpacing{\ZSFsubsubsectionBarBeforeSkip}{\BreakIfNotEnoughSpaceSubsubsection}%
  \ZSFBarPenalty{-300}%
  \if\relax\detokenize{#1}\relax\else\label{#1}\fi
...
%
%
\newcommand{\ZSFNewColumn}{\columnbreak[4]}
...
%
% Deshalb EIN `before` fuer den Vorabstand. Die Skip-Logik ist die von
% tcolorbox selbst — dieselben Faelle (nichts auf der Liste, vertikaler Modus),
% damit hier keine zweite Auslegung derselben Frage entsteht.
%
% DER BALKENABSTAND STEHT NICHT HIER, und das ist die eigentliche Trennlinie:
% Er haengt nicht an der Box, sondern an der Folge (was steht davor?), und wird
% deshalb beim Eintritt geholt — \ZSF@claimBarGap, aufgerufen von der Fabrik
% und von runintext. Der Vorabstand haengt umgekehrt an den OPTIONEN der Box
% (\ZSFBoxBeforeSkip, von der codebox pro Teil auf 0pt gezogen), und die kennt
% erst tcolorbox. Zwei Fragen, zwei Zeitpunkte — zusammengelegt bekam eine von
% beiden zwangslaeufig den falschen (Kopfkommentar zum Merker oben).
%
...
\newcommand{\ZSF@blockBefore}{%
  \ifZSF@barGapClaimed
    % Der Balkenabstand steht bereits ueber diesem Block; ein eigener
    % Vorabstand kaeme obendrauf. Das \par hat \ZSF@claimBarGap schon gesetzt.
  \else
    \ifnum\lastnodetype=-1\relax\else
      \par
      \ifvmode
        \ifdim\ZSFBoxBeforeSkip>0pt\relax
...
\tcbset{
  align/.is choice,
  align/left/.style  ={/utils/exec={\ZSFalignLefttrue\renewcommand{\ZSFBoxAlign}{\ZSFReadableAuto}}},
  align/center/.style={/utils/exec={\ZSFalignLeftfalse\renewcommand{\ZSFBoxAlign}{\centering}}},
...
%
%
\tcbset{ZSF@breakability/.style={breakable=false}}
...
% Muss in der Optionsliste NACH dem Basisstil stehen: Dessen gemeinsamer
% Breakability-Vertrag liest zuerst den Pack-Modus, `atomic` überschreibt ihn
% danach bewusst. Die Reihenfolge ist in den Presets festgeschrieben.
...
\NewDocumentEnvironment{ZSF@box}{m m m}{%
  ...
  % Ein ausstehender Balkenabstand gehoert dieser Box und wird HIER geholt —
  % in der aeusseren Liste und bevor tcolorbox die Box oeffnet. Danach steht
  % kein Merker mehr, den ihr Titel, ihr Inhalt oder eine Box in ihr sehen und
  % versehentlich verbrauchen koennte (Begruendung beim Merker oben).
  \ZSF@claimBarGap
...
\newcommand{\ZSFformulaline}[2]{%
  \ifZSFalignLeft
    \par\noindent$\displaystyle #1$\hfill\makebox[0pt][r]{\ZSFBoxNote{#2}}\par
  \else
    \par\noindent\hfill$\displaystyle #1$\hfill\makebox[0pt][r]{\ZSFBoxNote{#2}}\par
  \fi
}
...
\newtcbox{\GoalConditionBox}{%
  ...
  outer arc=\ZSFboxArc,
  left=\ZSFgoalPillPadX, right=\ZSFgoalPillPadX,
  top=\ZSFgoalPillPadY, bottom=\ZSFgoalPillPadY,
  nobeforeafter,
  tcbox raise base,
}
...
\newcommand{\GoalCondition}[1]{\ZSF@goalLink{\leavevmode\GoalConditionBox{\ZSFfontNote #1}\hfill}}
...
\newcommand{\ZSFvaluegridCommon}{
  ...
  \ZSF@gridLines
  rows={valign=m},
  cell{odd}{even}={bg=\TableZebraColor},
  cell{even}{odd}={bg=\TableZebraColor}
}
%
%
%
...
\NewDocumentEnvironment{valuegrid}{O{} O{} m}
  {\begingroup
   ...
   \edef\ZSF@vgOpts{colspec={*{#3}{X[c]}},\ZSFvaluegridCommon}%
...
\newcommand{\ZSFBoxesBreakableOn}{%
  \tcbset{ZSF@breakability/.style={breakable,break at=-\baselineskip/0pt}}%
}
...
% und war deshalb als zwei getrennte Hälften ein Fehler: Wer nur die Reserven
