...
\textVorBox{Die Formel der \ZSFkeyword[Ersatzfunktion, lineare@Ersatzfunktion, lineare]{linearen Ersatzfunktion} lautet:}
...
\textVorBox{Der \ZSFkeyword[Fehler, absoluter@Fehler, absoluter]{absolute Fehler} durch die Linearisierung beträgt:}
...
\begin{statementbox}[Mittelwertsatz]
...
\begin{statementbox}[Satz von Rolle]
Bei einer differenzierbaren, nicht injektiven Funktion $f: (a, b) \to \R$ existiert ein Punkt, dessen Ableitung $0$ beträgt.
\end{statementbox}
...
\begin{factlist}
    \ZSFFact{implizit}{$F(x,y) = 0$, nicht nach einer der beiden Variablen aufgelöst.}
    \ZSFFact{explizit}{$f(x)$, eine Variable steht allein auf einer Seite.}
\end{factlist}

$\Rightarrow$ Bei der Umrechnung von der Parameter- in die explizite Darstellung nach $t$ auflösen und gleichsetzen.

\textVorBox{Kurven können in \ZSFkeyword*{polaren Koordinaten} dargestellt werden \ZSFref{sec:hliddal_anhang}:}
