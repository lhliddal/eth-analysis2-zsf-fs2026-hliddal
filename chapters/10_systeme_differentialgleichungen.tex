...
\begin{procedure}[Lösungsverfahren: Homogene DGL-Systeme]
    ...
    \ProcStep{Eigenwerte/-vektoren}{Eigenwerte $\lambda_i$ und Eigenvektoren $\vec v_i$ bestimmen (Abkürzung siehe [10.4]).}
...
Eigenvektor $\vec v = \vec u \pm i \vec w$ in Realteil $\vec u$ und Imaginärteil $\vec w$ aufteilen. Die reelle Lösung ist dann direkt:}
\begin{formulabox}
    $\displaystyle \vec x = C_1 e^{at} \Big( \vec u \cos(bt) - \vec w \sin(bt) \Big) + C_2 e^{at} \Big( \vec u \sin(bt) + \vec w \cos(bt) \Big)$
\end{formulabox}

\textVorBox{Ist Matrix $A$ \ZSFkeyword{nicht diagonalisierbar} (nur ein Eigenvektor $\vec v$), bestimme den \ZSFkeyword{Hauptvektor} $\vec h$ via $(A - \lambda I)\vec h = \vec v$:}
\begin{formulabox}
    $\displaystyle \vec x = C_1 e^{\lambda t} \vec v + C_2 e^{\lambda t} (\vec h + t \vec v)$
\end{formulabox}

\textVorBox{Bei einem \ZSFkeyword{inhomogenen} System $\dot{\vec x} = A\vec x + \vec b(t)$ wählt man einen vektoriellen Ansatz (Koeffizientenvergleich):}
\begin{formulabox}
...
System umschreiben zu ${\dot{\vec x}=A\vec x}$. Eigenwerte $\lambda_i$ von $A$ (vgl. [10.4]) bestimmen Typ des Gleichgewichtspunkts:
...
\SubsectionBar{Eigenwerte \& Eigenvektoren}
