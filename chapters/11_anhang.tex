% ============================================================
% ANHANG-SCOPE: SEPARATE REGELN
% ============================================================
% !! WICHTIG !! Diese Datei wird NUR auf expliziten Nutzer-Befehl
% angefasst. Bei globalen Style-Aenderungen NIE diesen Anhang
% "mit-refaktorieren" — stattdessen am Header unten kompensieren.
% Siehe MODULAR_SYSTEM.md > "Anhang-Sonderregelung (PFLICHT)".
%
% Der Anhang folgt eigenen Layout-Regeln, unabhaengig vom Rest des
% Dokuments. Aenderungen am globalen Spacing/Box-Verhalten duerfen den
% Anhang NICHT beeinflussen. Wenn globale Werte angepasst werden, muss
% hier lokal kompensiert werden.
%
% Regeln:
%   1. Anhang darf dichter gesetzt sein als die Theoriekapitel.
%   2. Boxen muessen ueber Spaltengrenzen brechen koennen.
%   3. Keine \BreakIfNotEnoughSpace-Reserven (kein Whitespace am Spalten-
%      ende). Alle Display-Math-Bloecke duerfen brechen.
%   4. Spacing-Skala (ZSFspaceS/M/L) wird hier auf ihre dichten Original-
%      werte zurueckgesetzt, damit Formelboxen weiterhin spaltenbuendig
%      packen.
% ============================================================
% Spacing-Skala VOR \StartChapter zuruecksetzen, damit ChapterBar bereits
% mit den dichten Werten gerendert wird.
\setlength{\ZSFspaceS}{2pt}
\setlength{\ZSFspaceM}{4pt}
\setlength{\ZSFspaceL}{8pt}
% Boxen muessen brechen koennen — defbox/goalbox/formulabox/etc. im Anhang
% explizit auf breakable setzen (override der globalen unbreakable-Defs).
\tcbset{breakable}
\tcbset{zsftitlebox/.append style={breakable}}
\tcbset{zsfpanelbox/.append style={breakable}}
\StartChapter{Anhang}
...
\ZSFindexsee{Stammfunktionen}{Integraltabelle}
\begin{formulabox}[breakable]
    ...
    \]
    \[
    \int \frac{x}{x+\textcolor{green!60!black}{b}}\,dx = x - \textcolor{green!60!black}{b}\ln(|x+\textcolor{green!60!black}{b}|)+C
    \]
    \[
    ...
    \]
    \[
    \int \frac{1}{\textcolor{cyan}{a}^2-x^2}\,dx = \frac{1}{2\textcolor{cyan}{a}}\ln\!\left(\left|\frac{\textcolor{cyan}{a}+x}{\textcolor{cyan}{a}-x}\right|\right)+C
    \]
    \[
...
\begin{formulabox}[breakable, break at=-\baselineskip/0pt]
...
\SubsectionBar*{Anhang 4.1: Mitternachtsformel}
...
\SubsectionBar*{Anhang 5: Potenz-, Wurzel- und Logarithmusregeln}
\begin{valuegridcustom}{*{3}{X[c]}}
    $a^n \cdot a^m = a^{n+m}$ & $\frac{a^n}{a^m} = a^{n-m}$ & $(a^n)^m = a^{n \cdot m}$ \\
    $a^n \cdot b^n = (ab)^n$  & $a^{-n} = \frac{1}{a^n}$ & $a^{\frac{n}{m}} = \sqrt[m]{a^n}$ \\
    $\ln(x \cdot y) = \ln x + \ln y$ & $\ln\left(\frac{x}{y}\right) = \ln x - \ln y$ & $\ln(x^y) = y \ln x$ \\
    $a^x = e^{x \ln a}$ & $\log_b(x) = \frac{\ln x}{\ln b}$ & $e^{\ln x} = x \ (x > 0)$ \\
\end{valuegridcustom}

\SubsectionBar*[sec:hliddal_anhang]{Hliddal's Anhang}
...
\begin{fulltablebox}[Koordinaten-Umrechnung]{c | C !{\vrule width 1.1pt} C !{\vrule width 1.1pt} C}
    \TableTitle{ } & \TableTitle{Kartesisch} & \TableTitle{Zylindrisch} & \TableTitle{Sphärisch} \\
...
{\scriptsize
\ZSFindex{Volumen und Oberflaeche}
\begin{fulltablebox}[Wichtige Volumen und Oberflaechen]{c | C | C}
    \TableTitle{Koerper} & \TableTitle{Volumen $V$} & \TableTitle{Oberflaeche $O$} \\
    \noalign{\hrule height 1.1pt}
    \ZSFrowColor Quader & $abc$ & $2(ab+ac+bc)$ \\ \hline
    Achsenabschnitts-Tetraeder & $\frac{1}{6}abc$ & $\frac{1}{2}(ab+ac+bc)+\frac{1}{2}\sqrt{a^2b^2+a^2c^2+b^2c^2}$ \\ \hline
    Prisma & $A_G h$ & $2A_G+U_G h$ \\ \hline
    \ZSFrowColor Zylinder & $\pi r^2 h$ & $2\pi r(r+h)$ \\ \hline
    Kegel & $\frac{1}{3}\pi r^2 h$ & $\pi r(r+s)$ mit $s=\sqrt{r^2+h^2}$ \\ \hline
    \ZSFrowColor Kegelstumpf & $\frac{\pi h}{3}(R^2+Rr+r^2)$ & $\pi(R+r)s+\pi(R^2+r^2)$, $s=\sqrt{(R-r)^2+h^2}$ \\ \hline
    Kugel & $\frac{4}{3}\pi r^3$ & $4\pi r^2$ \\ \hline
    \ZSFrowColor Kugelkalotte & $\frac{\pi h^2}{3}(3R-h)$ & $2\pi Rh+\pi(R^2-(R-h)^2)$ \\ \hline
