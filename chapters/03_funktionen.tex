\StartChapterOnNewColumn{Funktionen}
...
Eine Funktion ist \ZSFkeyword{surjektiv}, wenn jeder Wert im Zielbereich angenommen wird: $Z(f) = W(f)$

Eine Funktion ist \ZSFkeyword{injektiv}, wenn jede Horizontale den Graphen höchstens einmal schneidet: $f(x_1) = f(x_2) \Rightarrow x_1 = x_2$

$\Rightarrow$ Eine Funktion ist \ZSFkeyword{bijektiv}, wenn sie injektiv und surjektiv ist. Bijektive Funktionen besitzen eine \ZSFkeyword{Umkehrfunktion / Inverse} mit $W(f^{-1}) = D(f)$ und $D(f^{-1}) = W(f)$
...
\ZSFkeyword{Trigonometrische Funktionen} beschreiben Zusammenhänge zwischen Winkeln und Seitenverhältnissen.
...
\textVorBox{Vorgehen beim Berechnen von Grenzwerten:
\begin{enumerate}
    \item Konstanten können vor den Grenzwert gezogen werden
    \item Bei gewissen Termen (z.B. bei Ausdrücken im Argument eines Sinus oder Cosinus) lohnt es sich zu substituieren
    \item Falls $\ZSFlimAuto{x \to \infty} f(x) = 0$ und $\ZSFlimAuto{x \to \infty} g(x) = 0$ (oder $\pm\infty$), darf man \ZSFkeyword{Bernoulli-L'Hôpital} anwenden:
\end{enumerate}}
...
\begin{formulabox}
    $\displaystyle f(g(x)) = y \quad \Rightarrow \quad \left(f(g(x))\right)^{-1} = g^{-1}(f^{-1}(y))$
\end{formulabox}
